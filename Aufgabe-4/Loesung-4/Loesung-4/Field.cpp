...
int Field::getnumber() const
{
	...
	// Therefore check string first and only accept digit entries.
...
int Field::calculate_neighbors(int x, int y) const
{
...
void Field::print(bool debug) const
{
...
std::vector<Tile*> Field::get_neighbours(const int x, const int y)
{
	...
	for (int i = -1; i <= 1; i++)
	{
		for (int j = -1; j <= 1; j++)
		{
			...
			// Try catch for out of bound calculations and push all neighbours into vector
			try
			{
